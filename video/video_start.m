...
load("Data/rightdown/aligned_center_points.mat");
time=double(z)';
spatial=double(x)';
defl=double(y)';
% Normalize
max_time = max(time);
max_spatial = max(spatial);
max_defl = max(defl);
time=time/max(time);
...
writematrix(spatial, 'spatial.csv');
writematrix(defl, 'defl.csv');
...
% load and process from mat
load("Data/leftdown/aligned_center_points.mat");
time=double(z)';
spatial=double(x)';
defl=double(y)';
% Normalize
...
xyz=AxelRot([time,spatial,defl]',0, [1,0,0],[0,0,0])';
